...
3. **與 L1-L19 的對照關係**：這是 kit 專屬判斷，不必每次重做。
...
- **對照通用情境**：命中 L5 / 或標「不在 L1-L19 涵蓋範圍（機制缺失類）」
...
Veros repo（含本目錄）屬**內部限閱**，見根目錄 `README.md` 的「機密等級」一節——
...
L1-L19」，L11／L12／L13 都是這樣長出來的。

- **程式碼層級的通用模式** → 一律走 L1-L19 那條既有路徑，**不進本庫**。
...
### 階段 2：回饋（在 Veros 的一般 clone，不在關鍵路徑上）
...
```bash
cd <你自己的 Veros clone>      # 不是專案裡的 submodule 路徑
...
- [ ] 「對照通用情境」欄位已填，且確認這條確實屬於領域層級（該有但沒寫的機制），而不是應該
      改走 L1-L19 維護規則的程式碼層級通用模式
