...
3. **與 L1-L16 的對照關係**：這是 kit 專屬判斷，不必每次重做。
...
- **對照通用情境**：命中 L5 / 或標「不在 L1-L16 涵蓋範圍（機制缺失類）」
...
L1-L16」，L11／L12／L13 都是這樣長出來的。

- **程式碼層級的通用模式** → 一律走 L1-L16 那條既有路徑，**不進本庫**。
