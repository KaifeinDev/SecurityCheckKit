...
第一輪只跑到 Step 1 就直接比對原始發現；後續補跑完整流程，結果如下。

**分類結果**（419 筆）：A 0、B 37、C 377（含風格預分類 202）、D 5（全為 `unprotected-upgrade`）。
**人工複核發現 3 筆**：M1（Cyfrin L-2／情境 L14）、M2（Cyfrin L-4／情境 L15＋L3）、M3（**Cyfrin 未涵蓋**／情境 L16）。
**資安等級第四級**，閘門 exit code 4——5 筆高嚴重度未排除，屬內部工作版本。Step 3（加抑制註解）刻意跳過：這是第三方 repo，不應改動其原始碼，故報告中「忽略前／忽略後」數字相同。

分類過程中有一個值得記錄的判斷：`reentrancy-no-eth` 原本打算沿用「有 nonReentrant」的理由套用到全部 15 筆，實際 grep 後發現 `BullionNFT`／`BullionNFTEnum` **完全沒有** ReentrancyGuard，其安全性來自另一個理由（外部呼叫對象 `mtokenContract` 於 init 後不可替換、無 setter，且限 `onlyOperator`）。兩個檔案因此給了不同的 dev_note——這正是 SKILL.md 警告「不可只憑 detector 名稱推斷理由」的實例。

### 報告格式調整（本次一併修正）

原始報告把過程產物與最終結論混在一起，419 筆逐筆展開，真正需要行動的 8 筆被埋沒。依「報告只放最終產出」的原則調整：

- **B／C 類改為按「檢查器 × 判定理由」歸併**：C 只列涉及檔案（結論是不需處置），B 仍逐組列出全部 `file:line`（客戶仍須據以查核是否接受風險）。A／D 與人工發現維持逐筆完整列出。逐筆紀錄仍在 `classification.json`，重掃可對帳。
- **「複核提醒」章節不再列舉全部編號**：原本 169 筆的群組會印出 `編號209, 編號210, …` 佔掉整頁；改為顯示前 5 筆作為抽查入口 + 涉及檔案 + 理由摘要。
- 效果：`report.md` 390 行 → **251 行**，PDF 由 22 頁縮短，且 8 筆待行動項目在第 2 頁的總表即可看見。

### 順帶修掉的兩個 PDF bug

1. **識別字被靜默吃掉**：fpdf2 的 `markdown=True` 把**雙底線**當成斜體分隔符並消耗掉，`__gap` 會渲染成 `gap`。而 `__gap`（OZ storage gap）與 `__Xxx_init` 是 Solidity 報告的日常內容——在資安報告裡靜默改掉識別字比失去斜體嚴重得多。fpdf 的 markdown 沒有跳脫語法，故以零寬空格拆開分隔符對（視覺無差異、不再被解析）。已驗證單底線與單星號不是分隔符，`**粗體**` 保留。
2. **未註冊斜體字型導致整份 PDF 產不出來**：承上，一旦解析出斜體就會呼叫 `set_font(style="I")`，而只註冊了 Regular/Bold，直接拋 `FPDFException` 中止。CJK 字型多半沒有真正的斜體，故把 I／BI 別名到正體——一個沒人要求的強調不該讓整份報告產不出來。
3. 封面與頁首的「檢測日期」原本直接印 ISO 時間戳（`2026-08-12T22:47:07.263397`），改為只取日期。
